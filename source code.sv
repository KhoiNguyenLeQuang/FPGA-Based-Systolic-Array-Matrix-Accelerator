...
module matrix_system_top(
    ...
    // SYSTOLIC FEEDER LOGIC: FEEDS NUMBERS FROM 2
    // MATRIXES TO THE MATH OPERATIONS
...
reg [5:0] compute_cycle; 
    ...
    always @(posedge sys_clk) begin
        if (rst) begin
            ...
            // We need enough cycles for the wave to pass through (N inputs + N latency + margin)
            if (compute_cycle < 30) compute_cycle <= compute_cycle + 1;

            for (i = 0; i < N; i = i + 1) begin
                // SKEWING CALCULATION:
                // Row 'i' should start receiving data 'i' cycles later than Row 0.
                // The data index we need is: current_time - row_number
                data_idx = compute_cycle - i;

                // HANDLE MATRIX A (Rows)
                if (data_idx >= 0 && data_idx < N) begin
                    // Valid window: Fetch mem_A[Row * N + Col]
                    a_drive[(i*WIDTH) +: WIDTH] <= mem_A[i*N + data_idx];
                end else begin
                    // Outside window: Feed 0s (Padding)
                    a_drive[(i*WIDTH) +: WIDTH] <= 0;
                end

                // HANDLE MATRIX B (Columns)
                // Matrix B is entered Top-Down. Col 'i' is skewed by 'i' cycles.
                if (data_idx >= 0 && data_idx < N) begin
                    // Valid window: Fetch mem_B[Row * N + Col]
                    // Note: mem_B is stored row-major, so B[k][i] is mem_B[k*N + i]
                    // Here 'data_idx' acts as the Row index (k) of B being fed into Col i
